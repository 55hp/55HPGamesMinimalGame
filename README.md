...
- ✅ Componente autorato su Main Camera (2.5D Menu) in `02_Gameplay`. Transform e FOV sono authored a occhio da Franci sul device di riferimento (Samsung Galaxy S25 Edge, 1440x3120 portrait): posizione `(2, 18, 1)`, rotazione `(90, 0, 0)` (verticale sul pozzo), `fieldOfView` 80 — nessun offset da codice (`CameraPositionOffset` rimosso il 17/09, §9).
- Il FOV **non** viene più ricalcolato con un fit trigonometrico esatto sui bound del pozzo/HUD/safe area (versione precedente, rimossa il 22/09 — vedi §9). Ora scala come farebbe un `CanvasScaler`: il FOV verticale authored sopra è il valore di riferimento (catturato in `Awake()` da quello impostato in Inspector), e viene riscalato per l'aspect ratio corrente con `tan(FOV_corrente/2) = tan(FOV_riferimento/2) * (aspect_riferimento / aspect_corrente)` — mantiene costante l'inquadratura orizzontale (larghezza pozzo) a qualsiasi aspect ratio; su schermi più stretti/alti del riferimento mostra più profondità (mai meno larghezza), su schermi più larghi/bassi meno profondità (framing non garantito per aspect molto insoliti, es. tablet — fuori scope, accettato).
- `BlockoutWellConfig.HorizontalPaddingScreenFraction` e i campi `_wellOrigin`/`_hudTopReservedCanvasUnits`/`_hudBottomReservedCanvasUnits` di `BlockoutWellCamera` sono stati rimossi il 22/09: il padding e il margine HUD sono ora impliciti nel FOV authored, non servono più a runtime.
...
- ✅ **`BlockoutWellCamera`**: fatto (22/09) — Transform/FOV authored su Main Camera (2.5D Menu) in `02_Gameplay`, vedi §2. `_wellOrigin`/`_hudTopReservedCanvasUnits`/`_hudBottomReservedCanvasUnits` non esistono più, nessun collegamento richiesto.
