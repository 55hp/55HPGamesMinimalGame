...
namespace hp55games.Blockout.Config
{
    ...
    public sealed class BlockoutSkin : ScriptableObject, IConfigAsset
    {
        ...
        [Tooltip("Periodic Table GDD only: chemical symbol (e.g. \"C\"). Empty for non-element skins (Default, Profondita, Juicy Clear).")]
        [SerializeField] private string _elementSymbol;

        [Tooltip("Periodic Table GDD only: atomic number. 0 for non-element skins - also doubles as \"is this an element skin\" for shop queries (see BlockoutSkinService.GetElementShopEntries).")]
        [SerializeField] private int _atomicNumber;

        [Tooltip("Periodic Table GDD only: which material variant (Bezi's Phase 2 shaders) this element's pieces should render with. Unused by non-element skins.")]
